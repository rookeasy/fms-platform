...
export default function Home() {
  return (
    <main className="fop-blueprint fop-fade-in min-h-screen bg-[#0F172A] text-white">
      <header className="flex min-h-16 items-center justify-between border-b border-white/10 px-5 sm:px-8">
        ...
        <nav className="flex items-center gap-3 text-sm">
          ...
          <Link href="/login" className="inline-flex h-10 items-center gap-2 rounded-xl bg-[#D95A4E] px-4 font-semibold text-white">
            Sign in
            <ArrowRight size={16} />
          </Link>
        </nav>
      ...
      <section className="mx-auto grid min-h-[calc(100vh-4rem)] w-full max-w-7xl items-center gap-10 px-5 py-10 sm:px-8 lg:grid-cols-[1fr_460px]">
        <div className="max-w-3xl">
          ...
          <p className="mt-5 text-sm font-semibold tracking-[0.26em] text-[#E9A099]">{fuzionBrand.tagline}</p>
          <p className="mt-3 text-sm text-slate-300">{fuzionBrand.protectedMetric}</p>
          <div className="mt-8 max-w-2xl">
            <ProgressIndex score={88} label="FPP Progress Index™" size="lg" showDescriptions variant="dashboard" />
          </div>
          <div className="mt-8 flex flex-wrap gap-3">
            <Link href="/login" className="inline-flex h-12 items-center gap-2 rounded-xl bg-[#D95A4E] px-5 text-sm font-semibold text-white hover:bg-[#C94F44]">
              Go to login
              <ArrowRight size={16} />
            </Link>
        ...
        <div className="rounded-3xl border border-white/10 bg-white/[0.07] p-5 shadow-[0_24px_80px_rgba(0,0,0,0.4)] backdrop-blur-xl">
          ...
          <div className="mt-5 grid gap-3">
            {platformSignals.map((signal) => {
              ...
              return (
                <div key={signal.label} className="flex items-center justify-between rounded-xl border border-white/10 bg-[#111827]/70 p-4">
                  <div className="flex items-center gap-3">
                    <Icon size={18} className="text-[#E9A099]" />
